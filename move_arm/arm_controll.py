...
from Arm_Lib import Arm_Device  # Dofbotの実機ライブラリ
...
def main():
    ...
    # =====================================================================
    Arm = Arm_Device()
    time.sleep(0.1) # 起動安定待ち
    
    # =====================================================================
    # 2. ロボットの構造定義（IKPyのルール：必ず下から上の順に定義）
    # =====================================================================
    ...
    target_y = 0.00
    target_z = 0.47255
    step = 0.01  # 1回の移動量（1cm）

    # =====================================================================
    # 3. キーボードが押された時の処理関数
    # =====================================================================
    def press(key):
        nonlocal target_x, target_y, target_z
        ...
        for i, angle_rad in enumerate(computed_angles[1:6]):
            ...
            # 実機への命令送信
            # =====================================================================
            # Dofbotの関数は、左から順番に「ID 1, 2, 3, 4, 5, 6, 時間」となっています。
            # そのため、上記で作った変数をそのままの順番で並べるだけで完璧に連動します！
        Arm.Arm_serial_servo_write6(target_angles[1], target_angles[2], target_angles[3], target_angles[4], target_angles[5], 90, 1000)
        
        # 画面表示もIDの順番通りに直してスッキリ
        print(f"位置 ➔ X:{target_x*100:.1f}cm, Y:{target_y*100:.1f}cm, Z:{target_z*100:.1f}cm | "
              f"送信角度: [ID1:{target_angles[1]:.0f}°, ID2:{target_angles[2]:.0f}°, ID3:{target_angles[3]:.0f}°, ID4:{target_angles[4]:.0f}°, ID5:{target_angles[5]:.0f}°, ID6:90°]")
    # =====================================================================
    # 4. 操作の開始
    # =====================================================================
    
    
    # 最初にアームを初期位置（X:15cm, Y:0cm, Z:25cm）にゆっくり1秒かけて移動させておく
    # これにより、キーを押したときに急激にガクッと動くのを防ぎます
    press('w') # ダミーで一回実行して計算角度を作る
